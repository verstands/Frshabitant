...
const ScriptUpdate = () => {
    const [userInfo, setUserInfo] = useState<ScriptInterface[] | null>(null)
      ...
      const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
             await serviceScript.updateScript(userInfo);
          setLoading(false);
          navigate("/script");
        } catch (error: unknown) {
      ...
      {hasAccess("create") && (
        <div className="px-4">
          <div className="border-white m-3 bg-white p-10 rounded-[10px] shadow">
            <h2 className="font-bold text-[20px]">{id ? "Modifier le script" : "Créer un script"}</h2>
            <hr />
            <br />
